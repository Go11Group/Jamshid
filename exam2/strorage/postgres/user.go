...
func (repo UserRepository) CreateUpdate(id string, user model.User) error {
...
func (repo UserRepository) CreateDelete(id string) error {
	_, err := repo.Db.Exec("delete from users where id=$1", id)
	return err
}
func (repo UserRepository) GetAll(id string) ([]model.User, error) {
	...
	for rows.Next() {
		user := model.User{}
		err := rows.Scan(&user.Name, &user.Email, &user.Birthday, &user.Password, &user.CreatedAt, &user.UpdatedAt, user.DeletedAt)
		if err != nil {
			return nil, err
		}
