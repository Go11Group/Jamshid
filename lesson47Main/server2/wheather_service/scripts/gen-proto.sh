...
# Check if the protos directory exists
if [ ! -d "$PROTOS_DIR" ]; then
  echo "Error: Protos directory $PROTOS_DIR does not exist."
  exit 1
fi

# Remove old genproto directory and create a new one
rm -rf "$GENPROTO_DIR"
mkdir -p "$GENPROTO_DIR"

# Find all directories under the protos directory and run protoc on them
